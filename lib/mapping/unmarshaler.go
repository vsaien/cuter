...
var (
	valueNotSettable = errors.New("value is not settable")
...
func (u *Unmarshaler) unmarshalWithFullName(m Valuer, v interface{}, fullName string) error {
	...
	for i := 0; i < numFields; i++ {
		field := rte.Field(i)
		if usingDifferentKeys(field, u.key) {
			continue
		}
...
func (u *Unmarshaler) processAnonymousFieldOptional(field reflect.StructField, value reflect.Value,
	...
	for i := 0; i < fieldType.NumField(); i++ {
		...
		fieldKey, fieldOptions, err := ParseKeyAndOptions(u.key, subField)
		if err != nil {
			return err
		}
...
func (u *Unmarshaler) processFieldNotFromString(field reflect.StructField, value reflect.Value,
	mapValue interface{}, options []string, fullName string) error {
	...
	default:
		return u.processFieldPrimitive(field, value, mapValue, options, fullName)
	}
}
...
func (u *Unmarshaler) processFieldPrimitive(field reflect.StructField, value reflect.Value,
	...
	default:
		...
		case json.Number:
			return u.processFieldPrimitiveWithJsonNumber(field, value, v, fullName)
		default:
			if typeKind == valueKind {
				if err := validateValueInOptions(options, mapValue); err != nil {
					return err
				}

				return fillWithSameType(field, value, mapValue)
			}
		}
	}
...
func (u *Unmarshaler) processFieldPrimitiveWithJsonNumber(field reflect.StructField, value reflect.Value,
	v json.Number, fullName string) error {
	fieldType := field.Type
	...
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		...
		if fieldKind == reflect.Ptr {
			value.Elem().SetUint(uint64(iValue))
		} else {
...
func (u *Unmarshaler) processNamedFieldWithValue(field reflect.StructField, value reflect.Value,
	mapValue interface{}, key string, opts *FieldOptions, fullName string) error {
	if mapValue == nil {
	...
	case reflect.Array, reflect.Map, reflect.Slice, reflect.Struct:
		return u.processFieldNotFromString(field, value, mapValue, nil, fullName)
	default:
		if u.opts.fromString || opts.fromString() {
			...
			if len(options) > 0 {
				if !stringx.Contains(options, value.String()) {
					return fmt.Errorf(`error: value "%s" for field "%s" is not defined in opts "%v"`,
						value.String(), key, options)
				}
			}

			return fillPrimitive(field.Type, value, mapValue, fullName)
		}

		return u.processFieldNotFromString(field, value, mapValue, opts.options(), fullName)
	}
}

func (u *Unmarshaler) processNamedFieldWithoutValue(field reflect.StructField, value reflect.Value,
	opts *FieldOptions, fullName string) error {
	fieldKind := Deref(field.Type).Kind()
	...
	case reflect.Array, reflect.Map, reflect.Slice, reflect.Struct:
		if !opts.optional() {
			return u.processFieldNotFromString(field, value, emptyMap, opts.options(), fullName)
		}
	default:
		if !opts.optional() {
...
func (u *Unmarshaler) fillMap(field reflect.StructField, value reflect.Value, mapValue interface{}) error {
	...
	fieldType := reflect.MapOf(fieldKeyType, fieldElemType)
	mapValueType := reflect.TypeOf(mapValue)
	valueType := reflect.MapOf(mapValueType.Key(), mapValueType.Elem())
	if fieldType == valueType {
		value.Set(reflect.ValueOf(mapValue))
	} else {
		refValue := reflect.ValueOf(mapValue)
		targetValue := reflect.MakeMapWithSize(fieldType, refValue.Len())
		fieldElemKind := fieldElemType.Kind()
		...
		for _, key := range refValue.MapKeys() {
			...
			case reflect.Slice:
				...
				if err := u.fillSlice(fieldElemType, target.Elem(), keythData); err != nil {
					return err
				}
			...
			case reflect.Struct:
				target := reflect.New(dereffedElemType)
				if err := u.Unmarshal(keythData.(map[string]interface{}), target.Interface()); err != nil {
					return err
				}
...
func (u *Unmarshaler) fillSlice(fieldType reflect.Type, value reflect.Value, mapValue interface{}) error {
	...
	for i := 0; i < refValue.Len(); i++ {
		...
		default:
			switch v := ithValue.(type) {
			case json.Number:
				if err := SetValue(dereffedBaseKind, conv.Index(i), v.String()); err != nil {
					return err
				}
			default:
				conv.Index(i).Set(reflect.ValueOf(ithValue))
			}
		}
	}
...
func (u *Unmarshaler) fillSliceFromString(fieldType reflect.Type, value reflect.Value,
	mapValue interface{}, fullName string) error {
	var slice []interface{}
	if err := codec.JsonUnmarshalString(mapValue.(string), &slice); err != nil {
		return err
	}
	...
	for i := 0; i < len(slice); i++ {
		...
		switch v := ithValue.(type) {
		case json.Number:
			if err := SetValue(baseFieldKind, conv.Index(i), v.String()); err != nil {
				return err
			}
		default:
			conv.Index(i).Set(reflect.ValueOf(ithValue))
...
func fillPrimitive(fieldType reflect.Type, value reflect.Value, mapValue interface{}, fullName string) error {
	if !value.CanSet() {
		return valueNotSettable
	}
	...
	switch v := mapValue.(type) {
	case string:
		return SetValue(baseType.Kind(), value, v)
	case json.Number:
...
func fillWithSameType(field reflect.StructField, value reflect.Value, mapValue interface{}) error {
	if !value.CanSet() {
		return valueNotSettable
	}
...
func validateValueInOptions(options []string, value interface{}) error {
	if len(options) > 0 {
		...
		default:
			if !stringx.Contains(options, fmt.Sprintf("%v", value)) {
