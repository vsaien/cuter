...
func TestUnmarshalYamlBytesOptionalDefault(t *testing.T) {
	var c struct {
		Name string
		Age  int `yaml:",optional,default=1"`
	}
	content := []byte(`Name: liao`)
...
func TestUnmarshalYamlBytesDefaultOptional(t *testing.T) {
	var c struct {
		Name string
		Age  int `yaml:",default=1,optional"`
	}
	content := []byte(`Name: liao`)
...
func TestUnmarshalYamlBytesSliceStructPtrDefault(t *testing.T) {
	var c struct {
		People []*struct {
			Name  string
			Age   int
			Email string `yaml:",default=chaoxin@liao.com"`
		}
	}
	content := []byte(`People:
- Name: liao
...
func TestUnmarshalYamlBytesInnerAnonymousPartial(t *testing.T) {
	type (
		Deep struct {
			A string
			B string `yaml:",optional"`
		}
		Inner struct {
			Deep
			InnerV string `yaml:",optional"`
		}
	)

	var c struct {
		Value Inner `yaml:",optional"`
	}
	content := []byte(`Value:
  InnerV: chaoxin`)
...
func TestUnmarshalYamlBytesEmptyMap(t *testing.T) {
	var c struct {
		Persons map[string]int `yaml:",optional"`
	}
	content := []byte(`{}`)
