...
func (st *SearchTree) Add(route string, item interface{}) error {
	...
	return add(st.root, route[1:], item)
}

func (st *SearchTree) Search(route string) (SearchResult, bool) {
	if len(route) == 0 || route[0] != slash {
		return NotFound, false
	}

	var result SearchResult
	ok := st.next(st.root, route[1:], &result)
	return result, ok
}

func (st *SearchTree) next(n *node, route string, result *SearchResult) bool {
...
func addParam(result *SearchResult, k, v string) {
