...
func BenchmarkPatRouter(b *testing.B) {
	...
	router := NewPatRouter()
	router.Handle(http.MethodGet, "/coretask/:user/:name", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	}))
	w := &mockedResponseWriter{}
	r, _ := http.NewRequest(http.MethodGet, "/coretask/a/b", nil)
	for i := 0; i < b.N; i++ {
		router.ServeHTTP(w, r)
	}
}
