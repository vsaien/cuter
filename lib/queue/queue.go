...
import (
	...
	"github.com/vsaien/cuter/lib/errorx"
...
func NewQueue(producerFactory ProducerFactory, consumerFactory ConsumerFactory) *Queue {
	queue := &Queue{
		metrics:              traffic.NewMetrics(queueName),
		producerFactory:      producerFactory,
...
func (queue *Queue) Start() {
	...
	queue.producerRoutineGroup.WaitForDone()
	close(queue.channel)
	queue.consumerRoutineGroup.WaitForDone()
}
...
func (queue *Queue) consumeOne(consumer Consumer, message string) {
	threading.RunSafe(func() {
		...
		defer func() {
			...
			queue.metrics.Add(traffic.Task{
				Duration: duration,
			})
			logx.Infof("%s - %s", duration, message)
		}()
